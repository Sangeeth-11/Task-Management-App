...
function ViewTask({ id }) {
  ...
  const handleShow = async () => {
    if (logStatus) {
      ...
      setData(result.data)
      setDataUpdate({ title: result.data.title, description: result.data.description, status: result.data.status })
    } else {
            ...
            <div className="card-body">
              ...
              <p className="card-text m-0 mb">Status-{data.status}</p>
              {
                data.userId?._id === user?._id ?
                  ...
                  <button className="btn btn-outline-danger mt-3" onClick={handleClose}><i className='fa fa-xmark'></i> close</button>
              }
            </div>
          </div>
          {
            edit ?
              <div>
                ...
                <FloatingLabel
                  ...
                  label="Status"
                  className="mb-3">
                  <Form.Control type="text" placeholder="status" value={dataUpdate.status} onChange={(e)=>{setDataUpdate({ ...dataUpdate, status: e.target.value })}} />
                </FloatingLabel>
